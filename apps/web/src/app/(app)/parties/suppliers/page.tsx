...
export default function SuppliersPage() {
  ...
  const { data: nextCode } = useQuery<string>({
    ...
    queryFn: () => apiFetch<{ code: string }>('/suppliers/next-code').then((r) => r.code),
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['suppliers', 'next-code'] });
      setModalOpen(false);
      setEditing(null);
      setForm({});
    },
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Supplier' : 'New Supplier'} size="lg">
        <form onSubmit={(e) => { e.preventDefault(); setError(''); save.mutate(form); }} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Code" hint="Auto-generated — it cannot be edited."><Input readOnly className="bg-slate-100 text-slate-500" value={form.code ?? (editing ? '' : (nextCode ?? ''))} onChange={(e) => set('code', e.target.value)} placeholder="Auto-generated" /></Field>
...
function SupplierDetail({
  ...
  const { data: ledgerResp, isLoading: ledgerLoading } = useQuery<{ entries: Array<{ id: string; debit: string | number; credit: string | number; runningBalance: string | number; description?: string | null; voucher?: { number: string; voucherDate: string; description?: string | null } | null }> }>({
    queryKey: ['supplier-ledger', id, ledgerView, ledgerFrom, ledgerTo],
    queryFn: () => apiFetch(`/suppliers/${id}/ledger${qs({ pageSize: ledgerView === 'summary' ? 10000 : 100, from: ledgerFrom || undefined, to: ledgerTo || undefined })}`),
    enabled: !!id,
  });
