...
export default function CustomersPage() {
  ...
  return (
    <div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); setError(''); save.mutate(form); }} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Code" hint="Auto-generated — it cannot be edited.">
              <Input readOnly className="bg-slate-100 text-slate-500" value={form.code ?? (editing ? '' : (nextCode ?? ''))} onChange={(e) => set('code', e.target.value)} placeholder="Auto-generated" />
            </Field>
      ...
      <CustomerDetail customer={detail} tab={detailTab} onTab={setDetailTab} onClose={() => setDetail(null)} />
      ...
      {!customer ? null : (
        <div>
          ...
          <div className="mb-4 flex gap-1 border-b border-slate-200">
            {(['ledger', 'sales', 'returns'] as const).map((t) => (
              <button
                key={t}
                onClick={() => onTab(t)}
                className={tab === t
                  ? 'border-b-2 border-teal-600 px-3 py-2 text-sm font-medium text-teal-700'
                  : 'border-b-2 border-transparent px-3 py-2 text-sm text-slate-500 hover:text-slate-700'}
              >
                {t === 'ledger' ? 'Ledger' : t === 'sales' ? 'Sales' : 'Returns'}
              </button>
            ))}
          </div>
