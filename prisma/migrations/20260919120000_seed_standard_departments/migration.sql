-- Seed the standard department set so every company starts with the typical
-- departments. Each department can be turned on/off from Company Features via
-- its dynamic feature code (`hr.departments.<id>`), so a client can hide any
-- department they don't need.
--
-- Codes use the `DEPT-` prefix (not the auto `DEP-001` numbering sequence) so
-- the numbering counter is never affected and new auto codes never collide.
-- Inserts are idempotent: an existing department with the same code is kept.
